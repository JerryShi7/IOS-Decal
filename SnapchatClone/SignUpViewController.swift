        ...
        if email == "" || password == "" || name == "" || verifiedPassword == "" {
            ...
            Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
                if error == nil {
                    ...
                    let changeNameRequest = Auth.auth().currentUser?.createProfileChangeRequest()
                    changeNameRequest?.displayName = name
                    changeNameRequest?.commitChanges(completion: {(err) in
                        if let error = err {
                            let alertController = UIAlertController(title: "Congratulations!", message: "Sign up Successfully!", preferredStyle: .alert)
                            let defaultAction = UIAlertAction(title: "OK", style: .cancel, handler: nil)
                            alertController.addAction(defaultAction)
