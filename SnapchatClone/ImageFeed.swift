            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        var threadKey = ""
                        var pathKey = ""
                        if let username = posts[postKey]?.value(forKey: firUsernameNode) as? String {
                            userName = username
                        }
                        if let path = posts[postKey]?.value(forKey: firImagePathNode) as? String {
                            pathKey = path
                        }
                        if let thread = posts[postKey]?.value(forKey: firThreadNode) as? String {
                            threadKey = thread
                        }
                        if let date = posts[postKey]?.value(forKey: firDateNode) as? String {
                            dateKey = date
                        }
                        let post = Post(id: postKey, username: userName, postImagePath: pathKey, thread: threadKey, dateString: dateKey, read: ids.contains(postKey))
